...
export default function KeyMetricsSection() {
    return (
        <section className="mb-24 relative z-10 w-full">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {metrics.map((metric, index) => (
                    <motion.div
                        ...
                        className="clay-card rounded-xl p-8 shadow-clay-lg flex flex-col gap-4 border border-white bg-white/60"
                    >
                        <div className="flex justify-between items-start">
                            <p className="text-slate-900 font-semibold text-lg">{metric.label}</p>
                            <span className="material-symbols-outlined text-primary bg-primary/10 p-2 rounded-xl shadow-clay-sm">{metric.icon}</span>
                        </div>
                        <div className="flex flex-col">
                            <h3 className="text-5xl font-bold tracking-tight text-slate-900">{metric.value}</h3>
